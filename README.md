...
| 区域 / 指标 | 数据源 | 是否需密钥 | 状态 |
|---|---|---|---|
| 美国 宏观/驱动（工业增加值·实际利率·库存·零售·耐用品订单·公共建造·出口） | **FRED** | 免费密钥 | 配置密钥后全自动 |
| 德国/欧盟 工业生产指数、基本金属生产 | **Eurostat** | 无 | ✅ 已自动 |
| 各区域「自华钢材(HS72)进口」额 + 同比 | **UN Comtrade** | 无 | ✅ 已自动 |
| 美国 周度产能利用率 | AISI | 无 | 尽力解析，失败回退人工 |
| 其余（PMI、印度/越南本土指标等无免费 API 者） | 人工 | — | 在 `config/sources.mjs` 录入种子 |

> 标 ● 的格子=自动真实序列；标 ○=暂无免费 API，沿用人工读数。看板始终完整渲染。

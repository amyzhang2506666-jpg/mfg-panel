...
// ----------------------------------------------------------------------------
// DBnomics — 免费聚合器（无 key），转发 OECD / Eurostat / IMF 等。
// code 形如 'OECD/DSD_KEI@DF_KEI/IND.M.PRVM.GR.BTE.Y.GY'
// ----------------------------------------------------------------------------
export async function dbnomics(code) {
  const url = `https://api.db.nomics.world/v22/series/${code}?observations=1`;
  try {
    const j = await getJSON(url);
    const s = j.series?.docs?.[0];
    if (!s || !s.period || !s.value) return null;
    const out = s.period
      .map((p, i) => ({ date: normMonth(p), value: +s.value[i] }))
      .filter(o => o.date && Number.isFinite(o.value))
      .sort((a, b) => a.date.localeCompare(b.date));
    return out.length ? out : null;
  } catch (e) { console.warn(`  [dbnomics ${code.split('/').pop()}] ${e.message}`); return null; }
}
// '2026-04' | '2026-Q2' | '2026' → 'YYYY-MM-01'
function normMonth(p) {
  let m = /^(\d{4})-(\d{2})$/.exec(p);            if (m) return `${m[1]}-${m[2]}-01`;
  m = /^(\d{4})-Q([1-4])$/.exec(p);               if (m) return `${m[1]}-${String((+m[2]) * 3 - 2).padStart(2, '0')}-01`;
  m = /^(\d{4})$/.exec(p);                          if (m) return `${m[1]}-12-01`;
  return null;
}
...
// ----------------------------------------------------------------------------
// macroview.club — 各国「制造业 PMI」聚合（NBS官方/ISM/S&P Global），月度，免费。
// 取数需 Django CSRF：先 GET 页面拿 csrftoken cookie + 表单 token，再 POST /get-chart。
// 整轮取数只握手一次，结果缓存；失败返回 null（由上层回退 CLI/人工）。
// 序列 key：us_ism_pmi 美 · de 德 · fr 法 · gb 英 · jp 日 · in 印 · kr 韩 · cn_pmi 中 · br 巴 · ru 俄
// ----------------------------------------------------------------------------
const MV_UA = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36' };
let _mvCache;
function macroviewCountryPMI() {
  if (_mvCache) return _mvCache;
  _mvCache = (async () => {
    const ref = 'https://www.macroview.club/charts?name=country_pmi';
    const page = await fetch(ref, { headers: MV_UA });
    const setCookie = page.headers.get('set-cookie') || '';
    const html = await page.text();
    const tok = (html.match(/name="csrfmiddlewaretoken"\s+value="([^"]+)"/)
              || html.match(/csrfmiddlewaretoken\s*=\s*'([^']+)'/) || [])[1];
    const cookie = (setCookie.match(/csrftoken=([^;]+)/) || [])[1];
    if (!tok || !cookie) throw new Error('CSRF token/cookie 未取到');
    const r = await fetch('https://www.macroview.club/get-chart', {
      method: 'POST',
      headers: { ...MV_UA, 'Content-Type': 'application/x-www-form-urlencoded',
        'Referer': ref, 'X-Requested-With': 'XMLHttpRequest', 'Cookie': `csrftoken=${cookie}` },
      body: `key=country_pmi&csrfmiddlewaretoken=${encodeURIComponent(tok)}&start=&end=&num=`,
    });
    if (!r.ok) throw new Error(`POST HTTP ${r.status}`);
    const j = await r.json();
    return j?.country_pmi?.data || null;
  })().catch(e => { console.warn(`  [macroview] ${e.message}`); return null; });
  return _mvCache;
}
export async function macroview(seriesKey) {
  const data = await macroviewCountryPMI();
  if (!data || !data[seriesKey]) return null;
  const out = data[seriesKey]
...
export async function fetchSeries(spec, env = {}) {
  switch (spec.provider) {
    ...
    case 'dbnomics':  return dbnomics(spec.code);
    case 'comtrade':  return comtrade(spec);
    case 'macroview': return macroview(spec.code);
